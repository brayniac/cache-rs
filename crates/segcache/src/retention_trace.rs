...
//!
//! Added to compare this engine's merge retention against another
//! segment-structured cache holding the same working set with a different
//! size mix. Five hypotheses about where the two differ were falsified by
//! measuring outcomes; a miss ratio cannot tell "decided differently" from
//! "was shown different items", and this separates them.
//!
...
//!
//! The bucketing is duplicated verbatim from the other engine's copy of
//! this file. It has to be: a comparison across two implementations of the
//! measurement measures the implementations as much as the caches. Keep the
//! two in step or the table means nothing.
//!
//! # Why the accumulator is per thread
//!
//! Same reason as [`crate::clock`]: a merge pass runs on the caller's
//! thread, and a process-global would leak between tests that cargo runs in
//! parallel.
//!
